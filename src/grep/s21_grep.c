...
void find_line(char** argv, FILE* fp, char* pattern, int file_index, struct flags flag) {
    char line[BUFF] = {0};
    ...
    while (fgets(line, BUFF, fp) != NULL) {
        ++number_lines;
        if (flag.o && !flag.v) {
            status = regexec(&compiled, line, 1, pmatch, 0);
            ...
            while (status != REG_NOMATCH) {
                ...
                status = regexec(&compiled, ptr, 1, pmatch, REG_NOTBOL);
            }
        } else {
            status = regexec(&compiled, line, 1, pmatch, 0);
            if (flag.v) {
